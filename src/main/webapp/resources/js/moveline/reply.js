...
function selectReplyList(){
    ...
    // contextPath, movelineNo, userNo 전역 변수 사용
    $.ajax({
        ...
        success : function(rList){
            ...
            for(let reply of rList){
                ...
                replyWriter.append(profileImage , userName , replyDate);
...
function showInsertReply(parentReplyNo, btn){
    ...
    if(temp.length > 0){ // 답글 작성 textarea 가 이미 화면에 존재하는 경우 (답글 작성 중 다른 답글 작성)
        
        if(confirm("다른 답글을 작성 중입니다. 현재 댓글에 답글을 작성하시겠습니까?")){
            temp[0].nextElementSibling.remove(); // 버튼 영역부터 삭제
            temp[0].remove(); // textarea 삭제 (기준점은 마지막에 삭제 해야함)
        } else {
            return; // 취소시 함수를 종료시켜 답글이 생성되지 않게함
        }
    } 
    ...
    // append : 마지막 자식으로 추가 이므로 after 이용
    // after(요소) : 뒤쪽에 추가 / before : 앞쪽에 추가
...
function insertChildReply(parentReplyNo, btn){
    ...
    // 누가? userNo     로그인한 회원의 userNo --> loginUserNo
    // 어떤 내용?         textarea에 작성된 내용
    // 몇번 게시글?       현재 게시글 movelineNo --> movelineNo(전역변수)
    ...
    // 위 if문이 실행 암됨 == 답글 작성 시 -> ajax 답글 삽입
    $.ajax({
        ...
        success : function(r){
            if(r > 0){
                alert("답글이 등록 되었습니다.");
                selectReplyList(); // 댓글 목록 다시 비동기 조회
            }else{
                alert("답글 등록이 실패했습니다.");
            }
        },
